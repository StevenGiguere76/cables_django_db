...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--speed', help='The speed that was used for testing.')
        parser.add_argument('--end_port_num', help='Test modules end at this port number.')
        parser.add_argument('--no_qsa_used', action='store_true', help='QSA used at any point during this test run.')
        parser.add_argument('--breakout_ports', action='store_true', help='Breakout ports are used in this test run.')

    def _find_cable_module(self, ethtool_dict):
        try:
            found_module = CableModule.objects.get(
                pin_number__exact=ethtool_dict['vendor_pn'],
                serial_number__exact=ethtool_dict['vendor_sn'])
        except CableModule.DoesNotExist as e:
            raise e
        except CableModule.MultipleObjectsReturned as e:
            import ipdb
            ipdb.set_trace()
    ...
    def _already_tested(self, platform, cable_module, speed):
        # Check if this module, for this platform, was already tested TODAY
        try:
            temp_test_result = TestResult.objects.get(
                platform=platform, module=cable_module, date_tested=date.today(), speed=speed)
    ...
    def handle(self, *args, **options):
        ...
        for port_num in range(int(options['start_port_num']), int(options['end_port_num']) + 1):
            # Call 'ethtool -m' to find what is being used in the port, only need vendor info
            if not options['breakout_ports']:
                ethtool_out = self._device_sudo_command('ethtool -m swp{0} | grep -i vendor'.format(port_num))
            else:
                ethtool_out = self._device_sudo_command('ethtool -m swp{0}s0 | grep -i vendor'.format(port_num))
            ...
            except CableModule.MultipleObjectsReturned:
                # Can't load this module, log it and move on
                log.info("Could not load module in swp{0}, too many entries found.".format(port_num))
                continue
            except CableModule.DoesNotExist:
                log.info("Could not load module in swp{0}, no entries found.".format(port_num))
                continue

            # Get the current speed of the module
            # speed_tested = self._get_speed_used(port_num)
            speed_tested = int(options['speed'])

            # Is there already a test result for this module
            if self._already_tested(platform, cable_module_tested, speed_tested):
                log.info('{0} was already tested, skipping.'.format(cable_module_tested))
                continue

            # Ask user is QSA was used
            if options['no_qsa_used']:
                qsa_used_bool = False
            else:
                qsa_used = raw_input('{0} QSA Used? '.format(cable_module_tested))

                if qsa_used.lower() == 'yes'or qsa_used.lower() =='y':
                    qsa_used_bool = True
                else:
                    qsa_used_bool = False
            ...
            if pass_or_fail == 'FAIL':
                pass_or_fail_bool = False
                bug_id_url = raw_input('Please enter the URL of the bug-id for this module: ')
            else:
                pass_or_fail_bool = True
                bug_id_url = None

            # Create test result
            new_test_result = TestResult(
                platform=platform,
                module=cable_module_tested,
                test_passed=pass_or_fail_bool,
                speed=speed_tested,
                qsa_used=qsa_used_bool,
                bug_id=bug_id_url)
